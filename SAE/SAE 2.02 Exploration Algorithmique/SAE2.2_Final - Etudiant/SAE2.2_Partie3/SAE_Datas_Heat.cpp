...
std::vector<std::string>  SAE_Datas_Heat::listAllFiles()const {
    std::filesystem::path path("data2");
...
void SAE_Datas_Heat::convertDataInList()
{
    ...
    for (size_t iStation = 0; iStation < maxStation; iStation++) {
        size_t nb = 0;
        std::shared_ptr<CelluleStation> newCel = std::make_shared<CelluleStation>(vIDstation[iStation]);
        std::shared_ptr<CelluleStation> temp = std::make_shared<CelluleStation>(vIDstation[iStation]);
        for (size_t mois = 0; mois < 12; mois++) {
            temp->tab_arbres[mois] = calculDeltaT(mois, vStationTmoy[iStation], vStationTsigma[iStation]);
            if (temp->tab_arbres[mois].nbelem() != 0)
        ...
        std::cout << nb << "nb ici + station" << newCel->_ID << std::endl;
    }
...
Arbre SAE_Datas_Heat::calculDeltaT(size_t mois, const std::vector<float>& vTmoyStation, const std::vector<float>& vTSigmaStation)
{
    ...
    for (size_t moisLu = mois; moisLu < nbMonths() - 12; moisLu += 12) {
        ...
        if (std::abs(deltaT) < DeltAbe) {
            std::shared_ptr<CelluleArbre> newCelArbre = std::make_shared<CelluleArbre>(deltaT, annee, vTmoyStation[moisLu], vTSigmaStation[moisLu]);
