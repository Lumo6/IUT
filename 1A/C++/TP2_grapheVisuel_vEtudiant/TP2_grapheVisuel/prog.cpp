		...
		while (pLibgraph->waitForEvent(e))
		{
			...
			case evtKeyDown:
				...
				case 'K':
					...
					break;
				case 'F':
					if (etape == Etape::FordFulk)
						etape = Etape::Noop;
					else
						etape = Etape::FordFulk;
					break;
				case 'X':
					if (etape == Etape::GraphExemple)
						etape = Etape::Noop;
					else
						etape = Etape::GraphExemple;
					break;
				}
				break;
			case evtMouseDown:
				switch (etape)
				{
					...
					{
						if (point1Select) {
							tp.GetGraph().SetNodeStart(pPoint1);
					...
					case Etape::GraphExemple: 
					{
						tp.GetGraph().CreateExempleGraph();
						pLibgraph->askForRefresh();
						break;
					}
				}
