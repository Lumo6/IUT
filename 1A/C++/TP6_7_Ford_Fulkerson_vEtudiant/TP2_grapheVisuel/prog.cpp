		...
		while (pLibgraph->waitForEvent(e))
		{
			...
			case evtMouseDown:
				switch (etape)
				{
					...
					// On ajoute une arete lorsque le premier point a bien été sélectionné et qu'on a cliqué de nouveau
						if (!point1Select) {
					...
					{
						if (!point1Select) {
							for (auto& ele : tp.GetGraph().GetNodes())
								ele->SetColor(-1);
							point1Select = true;
							tp.GetGraph().SetNodeStart(tp.GetGraph().GetNodeNear(e.x, e.y));
							tp.GetGraph().GetNodeStart()->SetColor(1);
							pLibgraph->askForRefresh();
							break;
						}
						else {
							point1Select = false;
							tp.GetGraph().SetNodeEnd(tp.GetGraph().GetNodeNear(e.x, e.y));
							tp.GetGraph().GetNodeEnd()->SetColor(0);
							etape = Etape::Noop;
							tp.GetGraph().OptimisationFF();
							pLibgraph->askForRefresh();
							break;
						}
					}
